	...
	switch (msg->Data[0]) {
		...
		case GUN_CAN_MOTOR_VEL_CMD:
			...
			if (msg->DLC == GUN_CAN_MOTOR_VEL_LENGTH) {
				...
				static CloseLoopFlag last_flag = OPEN_LOOP;
				static bool first_cmd = true;
				
				if (last_motion_command == GUN_CAN_MOTOR_VEL_CMD && !first_cmd && (last_set_vel == vel) && (last_flag == flag)){
					break;
				}
				first_cmd = false;
				last_flag = flag;
		...
		case GUN_CAN_MOTOR_ACCEL_CMD:
			...
			if (msg->DLC == GUN_CAN_MOTOR_ACCEL_LENGTH) {
				...
				static s32 last_accel = MAX_ORIG_ACC;
				static bool first_cmd = true;
				if (!first_cmd && (last_accel == accel)){
					break;
				}
				first_cmd = false;
				last_accel = accel;
		...
		case GUN_CAN_MOTOR_MAXV_CMD:
			...
			if (msg->DLC == GUN_CAN_MOTOR_MAXV_LENGTH) {
				...
				static s32 last_velc = MAX_ORIG_VEL;
				static bool first_cmd = true;
				if (!first_cmd && (last_velc == velc)){
					break;
				}
				first_cmd = false;
				last_velc = velc;
		...
		case GUN_CAN_MOTOR_POS_CMD:
			...
			if (msg->DLC == GUN_CAN_MOTOR_POS_LENGTH) {
				...
				static s32 last_pos = 0;
				static bool first_cmd = true;
				if (last_motion_command == GUN_CAN_MOTOR_POS_CMD && !first_cmd && (pos == last_pos)){
					break;
				}
				first_cmd = false;
				last_pos = pos;
