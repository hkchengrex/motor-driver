...
#define PID_POS_KP 45643
#define PID_POS_KI 745
#define PID_POS_KD 294
#define PID_POS_KV 0//142

#define PID_NEG_KP 45643
#define PID_NEG_KI 745
#define PID_NEG_KD 294
#define PID_NEG_KV 0//142
...
volatile s32 curr_err = 0;
volatile s32 diff_err = 0;
volatile s32 inte_err = 0;
volatile s32 velc_err = 0;
volatile s32 last_dif = 0;

s32 update_pid(){
	s32 curr_vel = get_unit_vel();
	...
	s32 duty = (s64)(Kp*curr_err + Ki*inte_err + Kd*diff_err + Kv*velc_err)/PID_SCALE;
	...
	if ((duty!=0) && (ABS(curr_vel)>4096) && (SIGN(curr_vel) != SIGN(duty))){
		...
		s32 offset = ABS(curr_vel)*MOSFET_REVERSE_COEFF;
		if (duty > 0){
			s32 offseted = duty - offset;
			if (offseted > 0){
				duty = offseted;
			}else{
				duty = 1;
